...
df_boston = pd.read_sql("""SELECT firstName, lastName, jobTitle FROM employees e LEFT JOIN offices USING(officeCode) where city='Boston'""", conn)
df_boston
...
# testScript = pd.read_sql("""SELECT * from offices where officeCode not in (select officeCode from employees)""", conn)

df_zero_emp = pd.read_sql("""SELECT officeCode,count(employeeNumber) as employeeCount from offices o join employees e USING(officeCode) group by officeCode HAVING count(employeeNumber) < 1""", conn)

True if len(df_zero_emp) > 0 else False
# testScript
...
# CodeGrade step3
# Replace None with your code
df_employee = pd.read_sql("""SELECT firstName, lastName,city, state from employees e left join offices o USING(officeCode) order by firstName, lastName""", conn)
df_employee
...
# CodeGrade step4
# Replace None with your code
df_contacts = pd.read_sql("""SELECT c.contactFirstName, c.contactLastName, c.phone,
       e.employeeNumber AS EmployeeNumberRep
FROM customers c
...
# CodeGrade step6
# Replace None with your code
df_credit = pd.read_sql("""SELECT employeenumber,firstname, lastname,count(1) as numberOfCustomers from employees e join customers c on e.employeenumber=c.salesrepemployeenumber group by e.employeenumber HAVING AVG(c.creditLimit) > 90000 ORDER BY numberOfCustomers desc;""", conn)
...
# CodeGrade step7
# Replace None with your code
df_product_sold = pd.read_sql("""select productname,count(ordernumber) as numorders, sum(quantityordered) as totalunits from orders o join orderdetails od USING (OrderNumber) join products using(productcode) group by productcode Order by totalunits desc""", conn)
...
# CodeGrade step8
# Replace None with your code
df_total_customers = pd.read_sql("""Select productname, productcode, count(DISTINCT c.customerNumber) as numpurchasers from customers c join orders o USING(customerNumber) join orderdetails od USING(orderNumber) JOIN products p USING (ProductCode) group by p.productcode, p.productname order by numpurchasers desc""", conn)
df_total_customers
...
# CodeGrade step9
# Replace None with your code
df_customers = pd.read_sql("""select count(DISTINCT CUSTOMERNUMBER) as n_customers, o.officeCode, o.city from offices o join employees e USING(officeCode) join customers c ON e.employeenumber = c.salesrepemployeenumber group by o.officeCode, o.city""",conn)
...
JOIN underperforming_products up 
    ON od.productCode = up.productCode
GROUP BY 
    e.employeeNumber,
    e.firstName,
    e.lastName,
